...
import Chart from '@/Components/Dashboard/ServicesChart.vue';
...
const updateData = () => {
    loading.value = true;
    axios
        .get(route('admin.dashboard.bookingAddonServices'), {
            params: {
                from: props.from,
                to: props.to,
                store: props.stores,
...
function prepareData()
{
    ...
    legend.value = [];
    chartData.value.labels = data.value.chartData.labels;
    let chart = {
        backgroundColor: [],
        data: data.value.chartData.data
    }

    data.value.chartData.labels.forEach((l, k) => {
        let label = {
            value: data.value.chartData.data[k],
            percent: 0,
            label: l,
            bg: colors[k]
        }

        legend.value.push(label);
        chart.backgroundColor.push(colors[k])
    })

    chartData.value.datasets.push(chart);
}
    ...
    <div class="stat-card">
        <div v-if="!loading" class="flex flex-col h-full justify-between items-start gap-4">
            <div class="flex justify-between items-center flex-wrap gap-2 w-full flex-grow-0">
                <p class="stat-title">Servizi secondari</p>
            ...
            <template v-if="true">
                <div v-if="!listView" class="flex justify-center items-center flex-wrap flex-grow gap-10 w-full">
                    <div class="relative">
                        <Chart class="py-0 px-1 w-56" :data="chartData"/>
