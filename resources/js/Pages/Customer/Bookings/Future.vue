...
{
  Inertia.delete(route('booking.destroy', itemToDelete.value), {
    onSuccess: (res) => {
        ...
        <div class="block">
          <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
              <Link :href="route('customer.bookings.future')" class="border-bb-blue-500 text-bb-blue-500 whitespace-nowrap pt-4 pb-2 px-1 border-b-4 font-medium text-sm">App. futuri</Link>
              <Link :href="route('customer.bookings.past')" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap pt-4 pb-2 px-1 border-b-4 font-medium text-sm" aria-current="page">App. passati</Link>
            </nav>
          </div>
        </div>
      </div>
        ...
        <div v-for="booking in bookingsPage" class="bb-card-stylist p-4 flex justify-between items-center mb-4">
          ...
          <div>
            <div class="flex justify-start items-center gap-x-2">
              ...
              <button v-if="booking.canBeEdited" type="button" class="icon-button !bg-bb-lightblue !rounded-full !text-bb-blue-500" @click.prevent="deleteBookingModal(booking.id)">
                <TrashIcon />
              </button>
            </div>
          </div>
