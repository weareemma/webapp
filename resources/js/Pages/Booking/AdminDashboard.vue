<template>
  <AppLayout title="Booking admin" :show-title="false">
...
<script setup>
import {ref, computed, onMounted, onUpdated, reactive, provide} from "vue";
...
import StepSuccess from "./Wizard/StepSuccess.vue";
import AdminBookingSideBar from "./Wizard/WizardComponents/AdminBookingSidebar.vue";
...
onMounted(() => {
  helpers.lg(props.originalBooking);
  ...
  {
    wizardSelection.value.customer_id = props.customer_id;
  }
  ...
  // Init store services
  wizardSelection.value.washing_stations = props.washing_stations;
...
function getMenuItemData(wizardStep) {
  switch (wizardStep) {
    case "step_customer":
      if (wizardSelection.value.customer_id) {
        return wizardGeneral.value.customers[wizardSelection.value.customer_id];
      }
      return "-";
    ...
    case "step_primary_hair_service":
      _.forEach(people.value, (p) => {
        data.push({
          ...
          data: [{
            title: ( p.primary_service ) ? (p.primary_service.title + '(' + p.primary_service.execution_time + "')") : null,
